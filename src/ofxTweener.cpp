...
Tween * ofxTweener::addTween(float &var, float to, float time){
	return addTween(var,to,time, &ofxTransitions::easeOutExpo ,0,0,false);
}

Tween *  ofxTweener::addTween(float &var, float to, float time, float (ofxTransitions::*ease) (float,float,float,float)){
	return addTween(var,to,time,ease,0,0,false);
}
Tween *  ofxTweener::addTween(float &var, float to, float time, float (ofxTransitions::*ease) (float,float,float,float), float delay){
	return addTween(var,to,time,ease,delay,0,false);
}
Tween *  ofxTweener::addTween(float &var, float to, float time, float (ofxTransitions::*ease) (float,float,float,float), float delay, float bezierPoint){
	return addTween(var,to,time,ease,delay, bezierPoint, true);
}
	
Tween * ofxTweener::addTween(float &var, float to, float time, float (ofxTransitions::*ease) (float,float,float,float), float delay, float bezierPoint, bool useBezier){
	float from = var;
	float _delay = delay;
...
void ofxTweener::update(){
	for(int i = tweens.size() -1; i >= 0; --i){
		if(float(tweens[i]->_timestamp.elapsed()) >= float(tweens[i]->_duration)){
            ...
            printf(" fillin\n");
            Tween * t = tweens[i];
            TweenEvent e = TweenEvent(t->_var);
            ofNotifyEvent(t->ON_COMPLETE, e);
